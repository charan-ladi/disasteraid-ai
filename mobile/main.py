...
class DisasterAidMobileApp(App):
    def build(self):
        ...
        api_layout = BoxLayout(orientation='horizontal', size_hint_y=None, height=40, spacing=5)
        ...
        self.select_btn = Button(text="Select Image / Audio File", on_release=self.show_file_chooser)
        self.upload_btn = Button(text="Process Report File", on_release=self.process_file, disabled=True)
        ...
        # Result Display Area
        self.root_layout.add_widget(Label(text="Structured JSON Result:", size_hint_y=None, height=20))
    ...
    def show_file_chooser(self, instance):
        ...
        file_chooser = FileChooserIconView(filters=['*.png', '*.jpg', '*.jpeg', '*.mp3', '*.wav', '*.pdf'])
        content.add_widget(file_chooser)

        select_layout = BoxLayout(orientation='horizontal', size_hint_y=None, height=40, spacing=10)
        ...
        def confirm_selection(btn_inst):
            if file_chooser.selection:
                ...
                self.file_label.text = f"Selected: {os.path.basename(self.selected_file_path)}"
